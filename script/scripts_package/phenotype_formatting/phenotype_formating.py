...
def binarisation_combinations (binarised_genotype_df, phenotype_df, phenotype_categories = None):
    ...
    if phenotype_categories: #this is branch for categorical data analysis only
        ...
        #this key we will use to get best accuracy for one category vs one category
    ...
    for i in range(top_n):
        ...
        value_best = combinations_for_binarisation_dict[key_with_highest_value]
        print(f'\33[34m\ntop {i+1} average accuracy: {accuracies[key_with_highest_value][0]:.2f}%, (penalised accuracy: {accuracies[key_with_highest_value][1]}) \n{value_best[1]} \33[37m')
...
def average_accuracy_from_dataframes(genotype_dataframe, phenotype_dataframe, coverage_restricted, max_coverage):
    ...
    accuracy_penalised = avg_acc * (0.8 + 0.2 * coverage_impact) # Balance the contribution, the higher the second coeficient is the higher coverage 

    return (avg_acc, accuracy_penalised)
